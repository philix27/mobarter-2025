...
export function Label(params: { children: ReactNode; className?: string }) {
  return (
    <div
      className={cn(
        'flex items-center justify-start mb-[2px] mt-1',
        params.className,
      )}
    >
      <p className="text-muted text-[12.5px] uppercase ml-1 font-light">
        {params.children}
      </p>
    </div>
  );
...
export const Line = () => (
  <hr className="border-[0.2px] h-[0.2px] border-muted" />
);
...
export const Card = (props: {
  ...
  <div
    className={cn(
      'bg-card rounded-md p-2  w-full hover:disabled text-muted',
      props.className,
    )}
